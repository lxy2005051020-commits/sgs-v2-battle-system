...
def make_context(seed: int = 7) -> BattleContext:
    return BattleContext(
        ...
        units={
            "a": UnitRuntime(
                "a", "A", "A", 100, 100, 100, 100, 10,
                lineup_position=LineupPosition.COMMANDER,
            ),
            "a2": UnitRuntime(
                "a2", "A2", "A", 100, 0, 100, 100, 20,
                lineup_position=LineupPosition.DEPUTY_1,
            ),
            "b": UnitRuntime(
                "b", "B", "B", 100, 100, 100, 100, 10,
                lineup_position=LineupPosition.COMMANDER,
            ),
            "b2": UnitRuntime(
                "b2", "B2", "B", 100, 100, 100, 100, 20,
                lineup_position=LineupPosition.DEPUTY_1,
            ),
        },
        event_bus=EventBus(),
...
def test_target_system_filters_living_units_and_uses_random_system() -> None:
    ...
    assert [
        unit.unit_id
        for unit in targets.random_units(
...
def test_target_system_full_selection_does_not_consume_randomness_and_uses_lineup_order() -> None:
    context = BattleContext(
        ...
        units={
            ...
            "b2": UnitRuntime(
                "b2", "B2", "B", 100, 100, 1, 1, 1,
                lineup_position=LineupPosition.DEPUTY_1,
            ),
            "b3": UnitRuntime(
                "b3", "B3", "B", 100, 100, 1, 1, 1,
                lineup_position=LineupPosition.DEPUTY_2,
            ),
            "b1": UnitRuntime(
                "b1", "B1", "B", 100, 100, 1, 1, 1,
                lineup_position=LineupPosition.COMMANDER,
            ),
        },
        event_bus=EventBus(),
...
def test_strategy_damage_requires_intelligence() -> None:
    context = make_context()
    systems = BattleSystems()
...
def test_damage_calculation_does_not_change_troops_until_troop_system_applies_it() -> None:
    ...
    damage = systems.damage_system.calculate(
        context,
        DamageRequest(
            source_id="a",
            target_id="b",
...
def test_normal_attack_routes_through_weapon_damage_request() -> None:
    context = BattleContext(
        ...
        units={
            "a": UnitRuntime(
                "a", "A", "A", 100, 100, 100, 100, 100,
                lineup_position=LineupPosition.COMMANDER,
            ),
            "b": UnitRuntime(
                "b", "B", "B", 100, 100, 100, 100, 90,
                lineup_position=LineupPosition.COMMANDER,
            ),
        },
        event_bus=EventBus(),
...
def test_context_rejects_mismatched_unit_key() -> None:
    with pytest.raises(ValueError, match="does not match"):
        BattleContext(
            ...
            units={
                "wrong": UnitRuntime(
                    "real", "A", "A", 100, 100, 1, 1, 1,
                    lineup_position=LineupPosition.COMMANDER,
                ),
                "enemy": UnitRuntime(
                    "enemy", "B", "B", 100, 100, 1, 1, 1,
                    lineup_position=LineupPosition.COMMANDER,
                ),
            },
            event_bus=EventBus(),
...
def test_context_rejects_duplicate_lineup_positions() -> None:
    with pytest.raises(ValueError, match="duplicate lineup position"):
        BattleContext(
            ...
            units={
                "a1": UnitRuntime(
                    "a1", "A1", "A", 100, 100, 1, 1, 1,
                    lineup_position=LineupPosition.COMMANDER,
                ),
                "a2": UnitRuntime(
                    "a2", "A2", "A", 100, 100, 1, 1, 1,
                    lineup_position=LineupPosition.DEPUTY_1,
                ),
                "a3": UnitRuntime(
                    "a3", "A3", "A", 100, 100, 1, 1, 1,
                    lineup_position=LineupPosition.DEPUTY_1,
                ),
                "b1": UnitRuntime(
                    "b1", "B1", "B", 100, 100, 1, 1, 1,
...
def test_battle_with_no_alive_units_ends_immediately_as_draw() -> None:
    context = BattleContext(
        ...
        units={
            "a": UnitRuntime(
                "a", "A", "A", 100, 0, 100, 100, 100,
                lineup_position=LineupPosition.COMMANDER,
            ),
            "b": UnitRuntime(
                "b", "B", "B", 100, 0, 100, 100, 100,
                lineup_position=LineupPosition.COMMANDER,
            ),
        },
        event_bus=EventBus(),
...
def test_commander_defeat_ends_battle_even_if_deputy_is_alive() -> None:
    context = BattleContext(
        ...
        units={
            "a1": UnitRuntime(
                "a1", "A主将", "A", 100, 100, 1000, 0, 200,
                lineup_position=LineupPosition.COMMANDER,
            ),
            "a2": UnitRuntime(
                "a2", "A副将", "A", 100, 100, 1, 1, 1,
                lineup_position=LineupPosition.DEPUTY_1,
            ),
            "b1": UnitRuntime(
                "b1", "B主将", "B", 10, 10, 1, 0, 100,
                lineup_position=LineupPosition.COMMANDER,
            ),
            "b2": UnitRuntime(
                "b2", "B副将", "B", 100, 100, 1, 1, 1,
                lineup_position=LineupPosition.DEPUTY_1,
            ),
        },
        event_bus=EventBus(),
