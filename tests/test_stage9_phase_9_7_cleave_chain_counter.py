...
def test_p97_clv_rec_01_share_target_death_recovery_uses_assigned_sum():
    ...
    assert result.recovery.attacker_recovery_basis == (
        result.partition_plan.dtarget + result.partition_plan.dsharer_theoretical
    )
    assert result.recovery.attacker_recovery_basis == 314
    assert received[0].attacker_recovery_basis == 314
...
def test_p97_clv_rec_02_distribution_participant_recovery_delegated():
    ...
    assert result.recovery.attacker_recovery_basis == result.actual_target_troop_loss == 50
...
def test_share_nonlethal_recovery_uses_assigned_partition_sum():
    ctx, systems = context(), BattleSystems()
    instance = cleave(ctx, systems, ratio=ExactRatio(1, 1))
