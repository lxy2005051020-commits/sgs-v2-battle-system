...
| :--- | :--- | :--- | :--- | :--- | :---: | :---: | :---: | :---: | :--- |
| **EM-01** | 目标裁决顺序 | 混乱压制嘲讽锁定，进入全场无差别目标池选择 | OBSERVED | `r11_confusion_taunt_data.json`<br>`raw_slices/EM-01_*` | 1,397 | 0 (82.8%切目标, 17.2%随机打嘲讽源) | **A** | A (无冲击) | 混乱全场无差别的精确权重分布（敌/友） |
...
| **EM-11** | 反击套反击防线 | 反击伤害不触发对方被动反击 (BLOCKED) | OBSERVED | `r7_recursion_denominators.json`<br>`raw_slices/EM-11_*` | 0 / 42 (有效分母42) | 0 | **B** | A (无冲击) | 样本分母 42 例，未见递归，评级定为 B |
...
| **EM-13** | 连环套连环防线 | 铁索连环反馈伤害绝不再次触发连环广播 (BLOCKED) | STATISTICALLY_SUPPORTED | `r7_recursion_denominators.json`<br>`raw_slices/EM-22_*` | 0 / 24,433 (有效分母24433) | 0 | **A** | A (无冲击) | 无 |
...
| **EM-25** | 连击重新索敌 | 连击第二击重新执行索敌，不固定继承第一击目标 | STATISTICALLY_SUPPORTED | `r10_combo_detailed_stratified.json` | 5,252 | 0 | **B** | A (无冲击) | 内部 PRNG 步进与权重分布属于 UNKNOWN |
...
全矩阵共 26 项核心机制断言，经过本次严格审计与等级挤压后：
* **Grade A (FROZEN FACT)**: **13 项** (50.0%)
* **Grade B (STRONG)**: **9 项** (34.6%)
