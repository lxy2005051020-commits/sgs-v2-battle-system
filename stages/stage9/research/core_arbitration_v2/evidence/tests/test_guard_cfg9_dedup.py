...
def test_inv05_guard_cfg9_dedup():
    lineup = {
        'my': [{'name': '关羽', 'pos': 1}],
        'enemy': [{'name': '曹操', 'pos': 1}, {'name': '许褚', 'pos': 2}]
    }
    ...
    events = [
        ...
        {'cfg_id': 143, 'clean_desc': '[曹操]执行来自[许褚]的「援护」效果', 'full_desc': ''},
        {'cfg_id': 9, 'clean_desc': '[关羽]对[许褚]发动普通攻击', 'full_desc': "<font color='#75b3ed'>[关羽]</font>对<font color='#ec616b'>[许褚]</font>发动普通攻击"},
        {'cfg_id': 28, 'clean_desc': '[许褚]损失了兵力100', 'full_desc': ''},
    ...
    assert attacks[0].resolved_target.display_name == '许褚'
    assert attacks[0].is_guard_redirected is True
